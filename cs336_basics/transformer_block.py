...
class TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.ffn = SwiGLU(d_model, d_ff=d_ff, device=device, dtype=dtype, activation=activation)
...
class TransformerBlockNoNorm(nn.Module):
    """
    与原 Pre-Norm 结构对齐，但用 nn.Identity 代替两处 RMSNorm（等价于移除）。
    """
    ...
    def forward(self, x: Tensor, token_positions: Optional[Tensor] = None) -> Tensor:
        h = self.input_layernorm(x)  # no-op
        attn_out = self.attn(h, token_positions=token_positions)
        x = x + attn_out
        h2 = self.post_attention_layernorm(x)  # no-op
        ffn_out = self.ffn(h2)
        x = x + ffn_out
        return x
...
class TransformerBlockPostNorm(nn.Module):
    ...
    与你的 Pre-norm 版本等价的组件，但把归一化位置移到 residual 之后。
    """
    ...
    def __init__(
        ...
        # 注意：post-norm 在每个子层 residual 之后做
        self.attn_layernorm = RMSNorm(d_model, device=device, dtype=dtype)

        self.ffn = SwiGLU(d_model, d_ff=d_ff, device=device, dtype=dtype)
        self.ffn_layernorm = RMSNorm(d_model, device=device, dtype=dtype)
    ...
    def forward(
        ...
        # ---- Self-Attention 子层：Residual -> Norm（post-norm）
        attn_out = self.attn(x, token_positions=token_positions)
        x = x + attn_out
        x = self.attn_layernorm(x)

        # ---- FFN 子层：Residual -> Norm（post-norm）
        ffn_out = self.ffn(x)
        x = x + ffn_out
