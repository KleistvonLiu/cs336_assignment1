...
uv run tests/train.py \
  ...
  --lr 1e-2 --min-lr 1e-3 --warmup-iters 200 --cosine-iters 5800 \
  --outdir ./runs_lr_single \
  --device cuda:0 \
  --log-to-csv \
  --diverge-patience 4
...
from tests.transformer_lm import TransformerLM, TransformerLMNoNorm, TransformerLMPostNorm


# ========== 轻量实验记录器（CSV + JSONL，可叠加 W&B） ==========
class CSVJSONLogger:
    def __init__(self, run_dir: str, enable_csv: bool = True, enable_jsonl: bool = True, flush_every: int = 1,
...
def run_one(args, lr_override: float | None = None, run_name_suffix: str = "") -> float:
    ...
    for it in range(start_iter, args.iters):
        ...
        if (it + 1) % args.log_interval == 0 or it == start_iter:
            ...
            last = wall
            print(
                f"[it {it + 1}/{args.iters}] lr={lr_t:.4e} loss={loss.item():.4f} gnorm={gnorm:.2f} ({dt * 1000:.1f} ms/iter)")
        ...
        if (it + 1) % args.eval_interval == 0:
            val_loss = _estimate_val_loss(model, val_ds, args.batch_size, args.context_length, device,
...
def main():
    ap = argparse.ArgumentParser(
        description="Train TransformerLM with AdamWFromScratch, cosine LR warmup, memmap data, checkpointing.")
    # 数据与设备
    ...
    # 消融实验
    ap.add_argument("--no-rmsnorm", action="store_true",
                    help="Remove all RMSNorms (use Identity) and train")
    ap.add_argument("--post-norm", action="store_true", help="Use post-norm Transformer blocks")
    ap.add_argument("--no-rope", action="store_true", help="Disable ROPE.")
    ap.add_argument("--activation", type=str, default="swiglu")
